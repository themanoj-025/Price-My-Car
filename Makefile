...
test: ## Run the helper unit tests inside the dev image
	$(DOCKER_COMPOSE) -f docker-compose.yml -f docker-compose.dev.yml run --rm app python -m pytest tests/test_helpers.py -v --tb=short -x

lint: ## Compile-check all Python files
	$(DOCKER_COMPOSE) -f docker-compose.yml -f docker-compose.dev.yml run --rm app sh -c "python -m compileall -q app/streamlit_app.py app/helpers.py && echo OK"
