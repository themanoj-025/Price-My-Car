...
COPY streamlit_app.py helpers.py prepare_ml_data.py ./
COPY Cleaned_Car_data.csv ./
...
COPY streamlit_app.py helpers.py test_helpers.py prepare_ml_data.py ./
COPY Cleaned_Car_data.csv ./
COPY ml_ready/ ./ml_ready/
COPY .streamlit/ ./.streamlit/
RUN python prepare_ml_data.py
