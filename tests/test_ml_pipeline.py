...
class TestModelLoading:
    ...
    def test_expected_model_names(self, all_models) -> None:
        ...
        assert must_have.issubset(set(all_models.keys())), f"Missing: {must_have - set(all_models.keys())}"
...
class TestEnsemble:
    ...
    def test_ensemble_single_model(self, preprocessor, sample_input) -> None:
        ...
        mean_pred, _spread, _color = ensemble_prediction({"Linear Regression": lr}, sample_input, preprocessor)
        assert mean_pred is not None
        assert mean_pred > 0
    ...
    def test_sorted_by_abs_contribution(self, all_models, preprocessor, sample_input, feature_names) -> None:
        ...
        for name, model in all_models.items():
            ...
            if len(result) > 1:
                ...
                assert abs_vals == sorted(abs_vals, reverse=True), f"{name}: not sorted by absolute contribution"
