...
Cleaned_Car_data.csv
        ↓
  prepare_ml_data.py (log1p transform, feature engineering)
        ↓
  ┌──────────────────────┬─────────────────┬──────────────────────┐
  │ generate_report.py   │ tune_hyperpara- │ train_dashboard_     │
  │ (HTML report, charts)│ meters.py       │ models.py            │
  │                      │ (GridSearchCV)  │ (tuned params)       │
  └──────────────────────┴─────────────────┴──────────────────────┘
                                  ↓
                        streamlit_app.py (auth + 9 pages + admin)
```
...
| File | Description |
|------|-------------|
| `prepare_ml_data.py` | Preprocessing, log-transform, feature engineering |
| `generate_report.py` | EDA report with visualizations |
| `create_notebook.py` | ML comparison notebook builder |
| `tune_hyperparameters.py` | GridSearchCV for ensemble models |
| `train_dashboard_models.py` | Trains tuned models for dashboard |
| `helpers.py` | Helper functions (testable, no Streamlit deps) |
| `test_helpers.py` | 65 unit tests |
| `streamlit_app.py` | Single-file dashboard (auth + 9 pages + admin) |
