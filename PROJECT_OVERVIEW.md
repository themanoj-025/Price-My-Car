...
#### `Price-My-Car/prepare_ml_data.py`
- **Purpose**: Preprocessing pipeline — log1p transform (skewness 5.64 → -0.12), feature engineering (39 features), train/test split.

#### `Price-My-Car/tune_hyperparameters.py`
- **Purpose**: GridSearchCV for ensemble models (XGBoost, Gradient Boosting, Random Forest).

#### `Price-My-Car/train_dashboard_models.py`
- **Purpose**: Trains tuned models and saves to `ml_ready/models/`.

#### `Price-My-Car/generate_report.py`
- **Purpose**: EDA report generation with visualizations.

#### `Price-My-Car/test_helpers.py`
...
2. `helpers.py:make_prediction()` preprocesses input
...
2. `helpers.py:compute_deal_score()` compares predicted vs asking price
...
```
streamlit_app.py → helpers.py → ml_ready/models/* → ml_ready/preprocessor.pkl
prepare_ml_data.py → Cleaned_Car_data.csv → ml_ready/
train_dashboard_models.py → tune_hyperparameters.py → ml_ready/models/
```
...
- **Framework**: pytest
- **Tests**: 65 unit tests in `test_helpers.py`
